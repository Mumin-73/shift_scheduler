...
if not os.path.exists("input"):
    os.makedirs("input")
if not os.path.exists("output/output_v1"):
    os.makedirs("output/output_v1")
if not os.path.exists("output/output_v2"):
    os.makedirs("output/output_v2")
        ...
        if created_files:
            ...
            with col2:
                if st.button("2단계: 자동 배정 실행 (v2)"):
                    ...
                    with zipfile.ZipFile("output/output_v2/result_bundle.zip", "w") as zipf:
                        for file in os.listdir("output/output_v2"):
                            if file.endswith(".xlsx"):
                                zipf.write(os.path.join("output/output_v2", file), arcname=file)
                    with open("output/output_v2/result_bundle.zip", "rb") as f:
