...
def save_results_to_excel(pretty_df: pd.DataFrame, total_hours: Dict[str, float], folder: str = "output/output_v2"):
    ...
    result_df.to_excel(os.path.join(folder, "총_근무시간표.xlsx"), index=False)

def main(output_dir="output/output_v2"):
    ...
    print(f"✅ 자동 배정 완료 → {output_dir} 폴더에 결과 저장됨")
